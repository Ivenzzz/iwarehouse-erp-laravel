        ...
        <div className="lg:hidden space-y-4">
          {requests.length === 0 ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800">
              ...
              <p className="text-sm">{searchTerm ? "No matching requests found" : "No stock requests yet"}</p>
            </div>
          ) : (
            requests.map((request) => (
              <div key={request.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-3 bg-white dark:bg-gray-800 text-xs text-gray-900 dark:text-gray-100">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-semibold text-gray-900 dark:text-white">{request.request_number || `Request #${request.id}`}</h3>
                    <p className="text-xs text-gray-600 dark:text-gray-400">{request.branch_name || "Unknown Branch"}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div><p className="text-xs text-gray-500 dark:text-gray-400">Requested By</p><p>{request.requested_by || "Unknown"}</p></div>
                  <div><p className="text-xs text-gray-500 dark:text-gray-400">Request Date</p><p>{request.created_at ? format(new Date(request.created_at), "MMM dd, yyyy | hh:mm a") : "N/A"}</p></div>
                  <div><p className="text-xs text-gray-500 dark:text-gray-400">Purpose</p><p className="line-clamp-1">{request.purpose}</p></div>
                  <div><p className="text-xs text-gray-500 dark:text-gray-400">Items</p><p>{request.total_items || 0}</p></div>
                </div>

                <div className="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-gray-700">
                  <span className="text-xs text-gray-500 dark:text-gray-400">Approved By: {request.approval_info?.approver_name || "N/A"}</span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">Required: {request.required_at ? format(new Date(request.required_at), "MMM dd, yyyy | hh:mm a") : "N/A"}</span>
                </div>
              </div>
            ))
          )}
              ...
              {requests.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-12 text-gray-500 dark:text-gray-400">
                    ...
                    <p>{searchTerm ? "No matching requests found" : "No stock requests yet"}</p>
                  </td>
                </tr>
              ) : (
                requests.map((request) => <StockRequestTableRow key={request.id} request={request} onPrint={onPrint} />)
              )}
            </tbody>
          </table>
        </div>
