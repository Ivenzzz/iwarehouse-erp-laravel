    ...
    return (
        <>
                ...
                <table className="min-w-full text-sm">
                    <thead className="sticky top-0 z-10 bg-table-header text-table-header-foreground backdrop-blur-sm">
                        <tr className="border-b border-border">
                            ...
                            <th className="px-4 py-3 text-left font-semibold text-foreground">
                                {sortableHeader('warehouse_type', 'Type')}
                            </th>
                            <th className="px-4 py-3 text-left font-semibold text-foreground">
                                {sortableHeader('city', 'City')}
                            </th>
                            <th className="px-4 py-3 text-left font-semibold text-foreground">
                                {sortableHeader('province', 'Province')}
                            </th>
                            <th className="px-4 py-3 text-left font-semibold text-foreground">Contact</th>
                            <th className="px-4 py-3 text-left font-semibold text-foreground">
                                {sortableHeader('sort_order', 'Sort Order')}
                            </th>
                    ...
                    <tbody className="bg-table-body text-table-body-foreground">
                        {warehouses.length > 0 ? (
                            warehouses.map((warehouse) => (
                                ...
                                >
                                    <td className="px-4 py-4 font-semibold text-foreground">{warehouse.name}</td>
                                    <td className="px-4 py-4 text-muted-foreground">{warehouse.warehouse_type}</td>
                                    ...
                                    <td className="px-4 py-4 text-muted-foreground">
                                        ...
                                        <div className="text-xs text-muted-foreground/80">{warehouse.email ?? 'No email'}</div>
                                    </td>
                                    <td className="px-4 py-4 text-muted-foreground">{warehouse.sort_order}</td>
                            ...
                            <tr>
                                <td colSpan={7} className="px-6 py-12 text-center">
                                    ...
                                    <p className="text-lg font-semibold text-foreground">
                                        {filters.search ? 'No warehouses found' : 'No warehouses yet'}
                                    </p>
            ...
            <div className="mt-4 flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                <p>
                    {pagination.total > 0
                        ? `Showing ${pagination.from} to ${pagination.to} of ${pagination.total} warehouses`
                        : 'Showing 0 warehouses'}
                </p>
